    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="reserva in reservas.data" :key="reserva.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/reservas/${reserva.id}/edit`">
            {{ reserva.id }}
            </Link>
          </td>
          <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/reservas/${reserva.id}/edit`" tabindex="-1">
          ...
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="`/reservas/${reserva.id}/edit`" tabindex="-1">
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/reservas', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
