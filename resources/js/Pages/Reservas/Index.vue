<template>
  <div :style="{ color: 'var(--color-text)', backgroundColor: 'var(--color-bg)' }" class="min-h-screen p-6">
    <Head title="Reservas" />
    <h1 class="mb-8 text-3xl font-bold" :style="{ color: 'var(--color-text)' }">Reservas / Cotizaciones</h1>
      ...
      <table class="w-full whitespace-nowrap transition-colors duration-300">
        ...
        <tbody>
          <tr v-for="reserva in reservas.data" :key="reserva.id" 
            ...
            <td class="px-6 py-4">
              <Link :href="`/inf513/grupo10sc/proyecto2/sis-maestranza/public/reservas/${reserva.id}/edit`" 
                    class="block w-full" :style="{ color: 'var(--color-text)' }">{{ reserva.id }}</Link>
            </td>
            <td class="px-6 py-4" :style="{ color: 'var(--color-text)' }">
              <Link :href="`/inf513/grupo10sc/proyecto2/sis-maestranza/public/reservas/${reserva.id}/edit`" tabindex="-1">
          ...
          <tr v-if="reservas.data.length === 0">
            <td class="px-6 py-4 border-t" :style="{ color: 'var(--color-text)' }" colspan="7">No se encontraron reservas.</td>
          </tr>
        </tbody>
      </table>
    </div>
